...
#define MLV_BLUE            lv_color_hex(0x2f3243)
...
//theme names first number color and back color
...
void display_state(uint32_t state)
{
	if(state == LS_FIREMAN_CONTROL)
	{
		...
		//lv_obj_set_style_local_image_recolor_opa(p_signal, LV_IMG_PART_MAIN, LV_STATE_DEFAULT, LV_OPA_TRANSP);
	}
...
{  
	...
	lv_obj_set_style_local_image_recolor_opa(p_main_bg, LV_IMG_PART_MAIN, LV_STATE_DEFAULT, FLOOR_NUM_OPA);
    lv_obj_set_style_local_image_recolor(p_main_bg, LV_IMG_PART_MAIN, LV_STATE_DEFAULT, FLOOR_NUM_COLOR);
...
static void main_page_run(void *p)
{
    ...
    while (1){
		...
		if(tick_loop-- == 0)
		{
			...
			//m	arcode_xfer(arcode_data);
			//m uint8_t len = arcode_data[0];

			//m uint8_t crc = arcode_crc(arcode_data);
			
			struct mikel_t mik;

			rt_memset(&arcodeLop,0,sizeof(arcodeLop));
				
			int len = mi_get(&mik, sizeof(mik));
	...
	//		else
	//		{
			//m if(crc == arcode_data[len-1])
			{


				// memcpy(&arcodeLop, &arcode_data[2],sizeof(arcodeLop));

				arcodeLop.leftCharacter = mik.Left;
				arcodeLop.rightCharacter= mik.Right;

				if(mik.Arrows == UP_DIRECTION)
					arcodeLop.travelUpDirection = 1;
				else if(mik.Arrows == DOWN_DIRECTION)
					arcodeLop.travelDownDirection = 1;

				if(mik.StringId == MCI_Fire_Alarm)
					arcodeLop.fireMode = 2;
				else if(mik.StringId == MCI_No_Service)
					arcodeLop.outOfService = 1;
				else if(mik.StringId == MCI_Maintenance)
					arcodeLop.outOfService = 1;
